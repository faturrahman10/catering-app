<x-app-layout>
    @section('page-title', 'Keuangan - Pemasukan Lain')
    ...
    <div class="p-4 md:p-6 lg:p-8">
        ...
        @if (session('success'))
            <div
                class="mb-6 bg-green-100 dark:bg-green-900/30 border border-green-400 dark:border-green-700 text-green-700 dark:text-green-400 px-4 py-3 rounded-lg">
                {{ session('success') }}
            </div>
        @endif
        ...
        @if (session('error'))
            <div
                class="mb-6 bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-400 px-4 py-3 rounded-lg">
                {{ session('error') }}
            </div>
        @endif
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Daftar Pemasukan Lain</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">Total: Rp
                    {{ number_format($totalIncomes, 0, ',', '.') }}</p>
            </div>
            <a href="{{ route('finance.incomes.create') }}"
                class="inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg text-sm font-medium hover:shadow-lg hover:shadow-green-500/50 transition-all duration-200">
        ...
        {{-- Filters --}}
        <div
            class="mb-6 bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-4">
            <form method="GET" action="{{ route('finance.incomes.index') }}" class="flex flex-wrap gap-4 items-end">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                    ...
                                    <div class="space-y-1">
                                        @foreach ($income->items as $item)
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span
                                                    class="text-xs text-gray-600 dark:text-gray-400">{{ $item['description'] }}</span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('finance.incomes.show', $income) }}"
        ...
        <div class="md:hidden space-y-4">
            ...
            @empty
                <div
                    ...
                    <p class="text-gray-600 dark:text-gray-400 font-medium">Tidak ada data pemasukan lain</p>
                </div>
            @endforelse
