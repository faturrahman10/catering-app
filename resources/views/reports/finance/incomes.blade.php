<x-app-layout>
    ...
    <div class="p-4 md:p-6 lg:p-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-2">
                        Laporan Pemasukan Lain
                    </h1>
                    <p class="text-gray-600 dark:text-gray-400 text-sm md:text-base">
                        Pemasukan selain dari penjualan orders
                    </p>
                </div>
        ...
        <div
            ...
            <form method="GET" action="{{ route('reports.incomes') }}" class="space-y-4">
        ...
        <div
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Total Pemasukan Lain</p>
        ...
        <div class="mb-6 flex gap-3">
            <a href="{{ route('reports.incomes.excel', request()->query()) }}"
            ...
            <a href="{{ route('reports.incomes.pdf', request()->query()) }}"
                        ...
                        @forelse ($incomes as $income)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                ...
                                <td class="px-6 py-4">
                                    <div class="space-y-1">
                                        @foreach ($income->items as $item)
                                            <div class="text-sm">
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400">
                                                    {{ $item['source'] }}
                                                </span>
                                                <span
                                ...
                                <td
                                    ...
                                    {{ count($income->items) }} item(s)
                                </td>
                                <td
                                    class="px-6 py-4 text-sm text-right font-semibold text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    @if ($income->proof_image)
                                        <a href="{{ Storage::url($income->proof_image) }}" target="_blank"
                    ...
                    @if ($incomes->isNotEmpty())
                        <tfoot class="bg-gray-50 dark:bg-gray-800/50">
                            <tr>
                                ...
                                <td
                                    class="px-6 py-4 text-sm font-bold text-right text-emerald-600 dark:text-emerald-400">
                                    Rp {{ number_format($totalIncomes, 0, ',', '.') }}
                                </td>
                                <td colspan="2"></td>
                            </tr>
        ...
        <div class="md:hidden space-y-4">
            @forelse ($incomes as $income)
                <div
                    ...
                    <div class="space-y-2 mb-3">
                        @foreach ($income->items as $item)
                            <div class="text-xs bg-emerald-50 dark:bg-emerald-900/10 rounded-lg p-2">
                                <span
                                    class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 mb-1">
                                    {{ $item['source'] }}
                                </span>
                    ...
                    @if ($income->notes)
                        <div class="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                <span class="font-medium">Catatan:</span> {{ $income->notes }}
                            </p>
                        </div>
                    @endif
                </div>
            @empty
                <div
                    ...
                    <p class="text-gray-500 dark:text-gray-400">Tidak ada data pemasukan lain</p>
                </div>
            @endforelse
        </div>
